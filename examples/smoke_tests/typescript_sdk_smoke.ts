...
async function expectApiError(
    ...
    try {
        ...
        if (!(err instanceof exc)) {
            bad(label, `wrong error type: expected ${exc?.name ?? 'Error'}, got ${err?.constructor?.name ?? typeof err}`);
            return;
        }
...
async function testMaxResultsValidations(): Promise<void> {
    ...
    for (const c of cases) {
        ...
        if (c.shouldSucceed) {
            ...
            await expectApiError(label, () => client.search({ query: 'rate limits and retries', maxResults: c.value }), {
