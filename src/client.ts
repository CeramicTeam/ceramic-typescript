...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CERAMIC_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CERAMIC_LOG'] or 'warn' if it isn't set.
   */
...
export class Ceramic {
   ...
   * API Client for interfacing with the Ceramic API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['CERAMIC_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['CERAMIC_BASE_URL'] ?? https://api.ceramic.ai/] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('CERAMIC_BASE_URL'),
    apiKey = readEnv('CERAMIC_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.CeramicError(
        "The CERAMIC_API_KEY environment variable is missing or empty; either provide it, or instantiate the Ceramic client with an apiKey option, like new Ceramic({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.ceramic.ai/`,
    };
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CERAMIC_LOG'), "process.env['CERAMIC_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('CERAMIC_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.ceramic.ai/';
  }
  ...
  static CeramicError = Errors.CeramicError;
...
export declare namespace Ceramic {
